...
while (scalar keys %$scanners > 0) {
    my ($found_id, $beacon_coords, $offsets) = find_match($scanners);
    add_beacon($_->[0], $_->[1], $_->[2]) for (@$beacon_coords);
    push @$distances, $offsets;
    delete $scanners->{$found_id};
}

my $combs = combinations($distances, 2);
my $max = 0;
while (my $c = $combs->next()) {
    my $dist = manhatten($c->[0], $c->[1]);
